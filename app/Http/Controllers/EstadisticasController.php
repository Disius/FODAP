...
class EstadisticasController extends Controller
{
    ...
    public static function docente_carrera()
    {
        ...
        for ($i = 0; $i < count($mecanica) - 1; $i++) {
            ...
            $total_m_mecanica = $mecanica[$i]->docente_inscrito->where('sexo', 1)->count() + $mecanica[$i + 1]->docente_inscrito->count();
            $total_f_mecanica = $mecanica[$i]->docente_inscrito->where('sexo', 2)->count() + $mecanica[$i + 1]->docente_inscrito->count();
        }
        $total_sistemas = 0;
        $total_m_sistemas = 0;
        $total_f_sistemas = 0;
